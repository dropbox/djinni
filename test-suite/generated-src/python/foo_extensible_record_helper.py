# AUTOGENERATED FILE - DO NOT MODIFY!
# This file generated by Djinni from foo_client_interface.djinni

from djinni.support import MultiSet # default imported in all files
from djinni.exception import CPyException # default imported in all files
from djinni.pycffi_marshal import CPyPrimitive, CPyRecord, CPyString
from PyCFFIlib_cffi import ffi, lib

from djinni import exception # this forces run of __init__.py which gives cpp option to call back into py to create exception

from foo_extensible_record import FooExtensibleRecord

class FooExtensibleRecordHelper:
    @staticmethod
    def release(c_ptr):
        assert c_ptr in c_data_set
        c_data_set.remove(ffi.cast("void*", c_ptr))

    @ffi.callback("int32_t(struct DjinniRecordHandle *)")
    def get_foo_extensible_record_f1(cself):
        try:
            _ret = CPyPrimitive.fromPy(CPyRecord.toPy(None, cself).number1)
            return _ret
        except Exception as _djinni_py_e:
            CPyException.setExceptionFromPy(_djinni_py_e)
            return ffi.NULL

    @ffi.callback("struct DjinniString *(struct DjinniRecordHandle *)")
    def get_foo_extensible_record_f2(cself):
        try:
            with CPyString.fromPy(CPyRecord.toPy(None, cself).string1) as py_obj:
                _ret = py_obj.release_djinni_string()
                assert _ret != ffi.NULL
                return _ret
        except Exception as _djinni_py_e:
            CPyException.setExceptionFromPy(_djinni_py_e)
            return ffi.NULL

    @ffi.callback("struct DjinniRecordHandle *(int32_t,struct DjinniString *)")
    def python_create_foo_extensible_record(number1,string1):
        py_rec = FooExtensibleRecord(
            CPyPrimitive.toPy(number1),
            CPyString.toPy(string1))
        return CPyRecord.fromPy(FooExtensibleRecord.c_data_set, py_rec) #to do: can be optional?

    @ffi.callback("void (struct DjinniRecordHandle *)")
    def __delete(dh):
        assert dh in FooExtensibleRecord.c_data_set
        FooExtensibleRecord.c_data_set.remove(dh)

    @staticmethod
    def _add_callbacks():
        lib.foo_extensible_record_add_callback_get_foo_extensible_record_f1(FooExtensibleRecordHelper.get_foo_extensible_record_f1)
        lib.foo_extensible_record_add_callback_get_foo_extensible_record_f2(FooExtensibleRecordHelper.get_foo_extensible_record_f2)
        lib.foo_extensible_record_add_callback___delete(FooExtensibleRecordHelper.__delete)
        lib.foo_extensible_record_add_callback_python_create_foo_extensible_record(FooExtensibleRecordHelper.python_create_foo_extensible_record)

FooExtensibleRecordHelper._add_callbacks()

