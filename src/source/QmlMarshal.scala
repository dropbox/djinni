...
class QmlMarshal(spec: Spec) extends Marshal(spec) {
  ...
  override def typename(tm: MExpr): String = toQmlType(tm, None, Seq())

  def typename(tm: MExpr, scopeSymbols: Seq[String]): String = toQmlType(tm, None, scopeSymbols)
  ...
  override def fqTypename(tm: MExpr): String = toQmlType(tm, Some(spec.cppNamespace), Seq())
  ...
  def cppFieldType(tm: MExpr): String = toCppType(tm, None, Seq())
  ...
  def hppReferences(m: Meta, exclude: String, forwardDeclareOnly: Boolean): Seq[SymbolReference] = m match {
    ...
    case MString => List(ImportRef("<QString>"))
  ...
  private def toQmlType(ty: TypeRef, namespace: Option[String] = None, scopeSymbols: Seq[String] = Seq()): String =
    toQmlType(ty.resolved, namespace, scopeSymbols)

  private def toQmlType(tm: MExpr, namespace: Option[String], scopeSymbols: Seq[String]): String = {
