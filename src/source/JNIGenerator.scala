...
class JNIGenerator(spec: Spec) extends Generator(spec) {
  ...
  override def generateEnum(origin: String, ident: Ident, doc: Doc, e: Enum) {
    ...
    writeJniHppFile(ident, origin, Iterable.concat(refs.jniHpp, refs.jniCpp), Nil, w => {
      w.wl(s"class $jniHelper final : ::djinni::JniEnum").bracedSemi {
        w.wlOutdent("public:")
  ...
  override def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: Record) {
    ...
    def writeJniPrototype(w: IndentWriter) {
      ...
      w.wl(s"class $jniHelper final").bracedSemi {
        w.wlOutdent("public:")
        w.wl(s"using CppType = $cppSelf;")
        w.wl(s"using JniType = jobject;")
        w.wl
        ...
        w.wl(s"~$jniHelper();")
        w.wl
        w.wl(s"static CppType toCpp(JNIEnv*, JniType);")
        w.wl(s"static ::djinni::LocalRef<JniType> fromCpp(JNIEnv*, const CppType&);")
        w.wl
        w.wlOutdent("private:")
        w.wl(s"$jniHelper();")
        w.wl(s"friend ::djinni::JniClass<$jniHelper>;")
    ...
    def writeJniBody(w: IndentWriter) {
      ...
      writeJniTypeParams(w, params) // TODO: this can't possibly be right
      w.wl(s"$jniHelper::$jniHelper() = default;")
      w.wl
      writeJniTypeParams(w, params)
      w.wl(s"$jniHelper::~$jniHelper() = default;")
      w.wl
      ...
      writeJniTypeParams(w, params)
      w.wl(s"auto $jniHelper::fromCpp(JNIEnv* jniEnv, const CppType& c) -> ::djinni::LocalRef<JniType>").braced{
      ...
      w.wl
      writeJniTypeParams(w, params)
      w.wl(s"auto $jniHelper::toCpp(JNIEnv* jniEnv, JniType j) -> CppType").braced {
        //w.wl(s"::${spec.jniNamespace}::JniLocalScope jscope(jniEnv, 10);")
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface) {
    ...
    def writeJniPrototype(w: IndentWriter) {
      ...
      w.wl(s"class $jniSelf final : $baseType").bracedSemi {
        ...
        w.wl
        if (i.ext.java) {
          w.wl(s"class JavaProxy final")
          w.wl(s": ::djinni::JavaProxyCacheEntry")
          w.wl(s", public $cppSelf").bracedSemi {
            w.wlOutdent(s"public:")
    ...
    def writeJniBody(w: IndentWriter) {
      ...
      w.wl
      writeJniTypeParams(w, typeParams)
      w.wl(s"$jniSelf::~$jniSelf() = default;")
      w.wl
      if (i.ext.java) {
        writeJniTypeParams(w, typeParams)
        w.wl(s"$jniSelf::JavaProxy::JavaProxy(JniType j) : JavaProxyCacheEntry(j) { }")
        w.wl
        writeJniTypeParams(w, typeParams)
        w.wl(s"$jniSelf::JavaProxy::~JavaProxy() = default;")
        w.wl
        for (m <- i.methods) {
          val ret = cppMarshal.fqReturnType(m.ret)
          val params = m.params.map(p => cppMarshal.fqParamType(p.ty) + " " + idCpp.local(p.ident))
          w.wl(s"$ret $jniSelf::JavaProxy::${idCpp.method(m.ident)}${params.mkString("(", ", ", ")")}").braced {
